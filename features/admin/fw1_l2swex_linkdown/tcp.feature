...
Feature: FW1 - L2SWEX 間リンク障害試験
  ...
  Scenario: リンクダウンの発生後のFailover確認[a]
    Given VPN サーバ
    And タジマックス社の PC
    When VPN サーバにタジマックス社の PC から TCP 接続を開始
    And 10 秒待つ
    And FW1 - L2SWEX 間リンク障害発生
    Then VPN サーバにタジマックス社の PC からの TCP 接続が維持されている
  ...
  Scenario: リンクダウンの発生後のFailover確認[b]
    ...
    And 社内の資産管理サーバ
    When 社内の資産管理サーバに DMZ の VPN アドレスプールから TCP 接続を開始
    And 10 秒待つ
    And FW1 - L2SWEX 間リンク障害発生
    Then 社内の資産管理サーバに DMZ の VPN アドレスプールから TCP 接続が維持されている
    And FW の主系が Passive 、予備系が Active になっていること
