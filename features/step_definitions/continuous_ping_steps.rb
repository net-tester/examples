...
When(/^VPN サーバにタジマックス社の PC から ping を連続実行$/) do
  cd('.') do
...
When(/^DMZ サーバに社内 PC から ping を連続実行$/) do
  cd('.') do
...
When(/^インターネット上の PC に社内 PC から ping を連続実行$/) do
  cd('.') do
...
Then(/^VPN サーバにタジマックス社の PC からの ping による疎通が (\d+) 秒以内に復帰$/) do |seconds|
  @ping_client.join
  cd('.') do
...
Then(/^DMZ のサーバに社内 PC からの ping による疎通が (\d+) 秒以内に復帰$/) do |seconds|
  @ping_client.join
  cd('.') do
...
Then(/^インターネット上の PC に社内 PC からの ping による疎通が (\d+) 秒以内に復帰$/) do |seconds|
  @ping_client.join
  cd('.') do
