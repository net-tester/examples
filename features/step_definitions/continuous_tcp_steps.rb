...
When(/^VPN サーバにタジマックス社の PC から TCP 接続を開始$/) do
  cd('.') do
...
When(/^社内の資産管理サーバに DMZ の VPN アドレスプールから TCP 接続を開始$/) do
  cd('.') do
...
Then(/^VPN サーバにタジマックス社の PC からの TCP 接続が維持されている$/) do
  @echo_client.join
  cd('.') do
...
Then(/^社内の資産管理サーバに DMZ の VPN アドレスプールから TCP 接続が維持されている$/) do
  @echo_client.join
  cd('.') do
