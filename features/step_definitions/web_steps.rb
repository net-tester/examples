...
When(/^ヨーヨーダイン社の VPN サーバに Web ブラウザから https でログイン$/) do
  cd('.') do
    ...
    system "sudo ip netns exec #{@vpn_host.name} echo '<title>LoginOK</title>' | sudo ip netns exec #{@vpn_host.name} openssl s_server -cert server.crt -key server.key -accept 443 > log/vpn_host.log &"
    sleep 2
